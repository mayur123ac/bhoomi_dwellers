...
export default function ReceptionistDashboard() {
  ...
  // ── Set up IntersectionObserver for table sentinel ──
  useEffect(() => {
    const sentinel = tableSentinelRef.current;
    if (!sentinel) return;
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && hasMore && !isLoadingMore && !isFetchingEnquiries) {
          loadMore();
        }
      },
      { threshold: 0.1 }
  ...
  // ── Set up IntersectionObserver for cards sentinel ──
  useEffect(() => {
  ...
  /** Rebuild pie chart from the full accumulated enquiries array */
  const rebuildChart = (allEnquiries: any[]) => {
    const configCounts: Record<string, number> = { "1 BHK": 0, "2 BHK": 0, "3 BHK": 0, "4+ BHK": 0, "Other": 0 };
    allEnquiries.forEach((item) => {
      const c = String(item.configuration || "").trim();
      if (configCounts[c] !== undefined) configCounts[c]++;
      else configCounts["Other"]++;
    });
    setChartData(
      [
  ...
  /** Fetch a single page of enquiries. append=true adds to existing list. */
  const fetchPage = async (currentOffset: number, append: boolean) => {
    try {
      ...
      // Support both { data: [], total: N } and bare []
  ...
  /** First load: reset everything and fetch page 0 */
  ...
  /** Triggered by sentinel visibility */
  ...
  const handleEnquirySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Re-fetch from scratch so the new entry (latest first) appears at top
        initialLoad();
      } else {
  ...
  const maskPhoneNumber = (phone: any) => {
    ...
    return `${cleanPhone.slice(0, 2)}${"*".repeat(cleanPhone.length - 5)}${cleanPhone.slice(-3)}`;
  };
  ...
  // Client-side filter on already-loaded entries
  ...
  // ─────────────────────────────────────────────────
  // Shared "load more" loader row/card
  // ─────────────────────────────────────────────────
  const LoaderRow = () => (
    <tr>
  ...
  const LoaderCards = () => (
    <div className="col-span-full flex items-center justify-center gap-3 text-gray-400 text-sm py-8">
      <div className="flex gap-1">
        <span className="w-2 h-2 rounded-full bg-purple-500 animate-bounce" style={{ animationDelay: "0ms" }} />
        <span className="w-2 h-2 rounded-full bg-purple-500 animate-bounce" style={{ animationDelay: "150ms" }} />
        <span className="w-2 h-2 rounded-full bg-purple-500 animate-bounce" style={{ animationDelay: "300ms" }} />
  ...
  const EndMessage = ({ count }: { count: number }) => (
    ...
    <div className="flex h-screen font-sans bg-[#121212] text-white overflow-hidden">
      ...
      <aside className="w-20 bg-[#1a1a1a] border-r border-[#2a2a2a] flex flex-col items-center py-6 flex-shrink-0 z-40 shadow-sm">
        ...
        <nav className="flex flex-col space-y-6 w-full items-center">
          {[
            { id: "overview", icon: <FaThLarge className="w-6 h-6" />, title: "Dashboard" },
            { id: "forms", icon: <FaClipboardList className="w-6 h-6" />, title: "Forms List" },
            { id: "assistant", icon: <FaRobot className="w-6 h-6" />, title: "CRM AI Assistant" },
          ].map(({ id, icon, title }) => {
            ...
            return (
              <div key={id} onClick={() => setActiveTab(id)} className="group relative flex justify-center cursor-pointer w-full" title={title}>
                {active && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-purple-500 rounded-r shadow-[0_0_10px_2px_rgba(168,85,247,0.5)]" />}
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all ${active ? "bg-purple-900/20 text-purple-400" : "text-gray-500 hover:bg-white/5 hover:text-gray-300"}`}>{icon}</div>
              </div>
            );
          })}
          <div onClick={() => setActiveTab("settings")} className="group relative flex justify-center cursor-pointer w-full mt-auto" title="Settings & Profile">
            {activeTab === "settings" && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-purple-500 rounded-r shadow-[0_0_10px_2px_rgba(168,85,247,0.5)]" />}
            ...
            {isProfileOpen && (
              <div className="absolute top-12 right-0 w-64 bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl shadow-2xl p-5 z-50 animate-fadeIn">
                ...
                <div className="space-y-4 mb-6 text-sm">
                  <p className="text-gray-400 flex justify-between items-center">Role: <span className="text-purple-400 font-bold capitalize bg-purple-500/10 px-2 py-0.5 rounded">{user?.role || "Employee"}</span></p>
                  <div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-between bg-[#121212] p-3 rounded-lg border border-[#2a2a2a]">
                        ...
                        <button onClick={() => setShowPassword(!showPassword)} className="text-gray-400 hover:text-purple-500 cursor-pointer">{showPassword ? <FaEyeSlash /> : <FaEye />}</button>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  {chatMessages.map((msg, idx) => (
                    <div key={idx} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                      <div className={`flex gap-3 max-w-[85%] ${msg.sender === "user" ? "flex-row-reverse" : "flex-row"}`}>
                        ...
                        <div className={`p-4 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${msg.sender === "user" ? "bg-blue-600 text-white rounded-tr-none" : "bg-[#222] text-gray-200 border border-[#2a2a2a] rounded-tl-none"}`}>{msg.text}</div>
                      </div>
                    </div>
                  ))}
                  <div ref={chatEndRef} />
          ...
          {activeTab !== "settings" && activeTab !== "detail" && activeTab !== "assistant" && (
            <div className="flex justify-between items-center mb-8">
              <h1 className="text-2xl md:text-3xl font-bold flex items-center flex-wrap gap-3">
                ...
                <span className="text-sm font-normal text-purple-400 bg-purple-500/10 border border-purple-500/30 px-3 py-1 rounded-full shadow-sm capitalize">{user?.role || "Employee"}</span>
              </h1>
              <button onClick={initialLoad} className="text-purple-400 text-sm font-semibold flex items-center gap-2 hover:text-purple-500 cursor-pointer bg-purple-500/10 px-4 py-2 rounded-lg transition-colors">↻ Refresh Live Data</button>
            </div>
          )}
              ...
              <div className="flex flex-col lg:flex-row gap-6 mb-8">
                ...
                <div className="w-full lg:w-1/3 bg-[#1a1a1a] rounded-2xl p-6 border border-[#2a2a2a] shadow-sm flex flex-col items-center">
                  ...
                  <p className="text-xs text-gray-400 self-start mb-6">
                    From {enquiries.length} loaded of {totalCount} total records
                  </p>
                  {isFetchingEnquiries ? (
                    ...
                    <div className="w-full h-[220px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          ...
                          <Tooltip contentStyle={{ backgroundColor: "#1a1a1a", border: "1px solid #333", borderRadius: "8px", color: "#fff" }} itemStyle={{ color: "#fff" }} />
                ...
                <div className="w-full lg:w-2/3 grid grid-cols-2 gap-6">
                  <div className="bg-[#1a1a1a] rounded-2xl p-8 border border-[#2a2a2a] shadow-sm flex flex-col justify-center relative overflow-hidden">
                    ...
                    <p className="text-5xl font-black mb-2 text-white">{isFetchingEnquiries ? "..." : totalCount}</p>
                ...
                <div className="p-6 border-b border-[#2a2a2a] flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      ...
                      {hasMore && <span className="text-purple-400"> · scroll for more</span>}
                    </p>
                  </div>
                ...
                <div className="overflow-x-auto custom-scrollbar">
                  <table className="w-full text-left border-collapse whitespace-nowrap">
                    <thead>
                      <tr className="bg-[#222]">
                        {["Lead No.", "Client Name", "Email ID", "Budget", "Phone", "Alt. Phone", "Date Created", "Sales Manager"].map((h) => (
                          <th key={h} className="p-4 text-xs text-gray-400 font-bold uppercase tracking-wider border-b border-[#2a2a2a]">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#2a2a2a]">
                      ...
                      {/* ── Loader row when fetching more ── */}
                      {isLoadingMore && <LoaderRow />}

                      {/* ── End-of-list message ── */}
                      {!hasMore && !isFetchingEnquiries && enquiries.length > 0 && (
                        <tr>
                          <td colSpan={8} className="p-4 text-center text-xs text-gray-600">
                  ...
                  {/* ── Invisible sentinel div that triggers load-more ── */}
            ...
            <div className="animate-fadeIn">
              ...
              {isFetchingEnquiries ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  {/* ── Loader cards when fetching more ── */}
                  {isLoadingMore && <LoaderCards />}

                  {/* ── End-of-list message ── */}
                  {!hasMore && !isFetchingEnquiries && enquiries.length > 0 && (
                    <div className="col-span-full">
                      <EndMessage count={totalCount} />
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="bg-[#1a1a1a] rounded-2xl border border-[#2a2a2a] p-8 shadow-xl">
                <div className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 text-white">
                  ...
                  <div className="sm:text-right">
                    ...
                    <p className="font-semibold flex items-center sm:justify-end gap-2"><FaBriefcase className="text-purple-300 opacity-70" /> {selectedEnquiry.source || "N/A"}</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-[#222] border border-[#333] rounded-xl p-5">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div><p className="text-xs text-gray-500 font-medium mb-1">Primary Source</p><p className="text-white font-medium text-sm">{selectedEnquiry.source || "N/A"}</p></div>
                        {selectedEnquiry.source === "Others" && (
                          <div><p className="text-xs text-gray-500 font-medium mb-1">Specified Name</p><p className="text-white font-medium text-sm">{selectedEnquiry.source_other}</p></div>
                        )}
                      </div>
                      {selectedEnquiry.source === "Channel Partner" && (
                        <div className="mt-4 pt-4 border-t border-[#333] grid grid-cols-1 sm:grid-cols-3 gap-4">
                          <div><p className="text-xs text-gray-500 font-medium mb-1">CP Name</p><p className="text-white font-medium text-sm">{selectedEnquiry.cp_name || "N/A"}</p></div>
                          <div><p className="text-xs text-gray-500 font-medium mb-1">CP Company</p><p className="text-white font-medium text-sm">{selectedEnquiry.cp_company || "N/A"}</p></div>
                          <div><p className="text-xs text-gray-500 font-medium mb-1">CP Phone</p><p className="text-white font-medium text-sm">{selectedEnquiry.cp_phone || "N/A"}</p></div>
                        </div>
                      )}
                    </div>
                  </div>
