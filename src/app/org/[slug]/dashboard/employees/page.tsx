...
export default function EmployeesPage() {
  ...
  useEffect(() => {
    ...
    // Use centralized RBAC: company_admin + admin = Organization Admin
    if (canManageEmployees(userRole) || isSiteHead(userRole)) {
  ...
  // Extract org slug from the current URL path: /org/[slug]/dashboard/employees
  const orgSlug = typeof window !== "undefined"
    ? window.location.pathname.split("/").filter(Boolean)[1] || ""
    : "";

  const menuItems = [
    { id: "dashboard", icon: FaThLarge, label: "Overview", link: `/org/${orgSlug}/dashboard`, section: null },
    { id: "receptionist", icon: FaClipboardList, label: "Receptionist", link: `/org/${orgSlug}/dashboard`, section: null },
    { id: "sales", icon: FaUsers, label: "Sales Managers", link: `/org/${orgSlug}/dashboard`, section: null },
    { id: "site_head", icon: FaUniversity, label: "Site Heads", link: `/org/${orgSlug}/dashboard`, section: null },
    { id: "monitoring", icon: FaChartPie, label: "Daily Monitor", link: `/org/${orgSlug}/dashboard`, section: null },
    { id: "callers", icon: FaPhoneAlt, label: "Caller Panel", link: `/org/${orgSlug}/dashboard/employees`, section: "callers" as const },
    { id: "employees", icon: FaIdCard, label: "Add Employee", link: `/org/${orgSlug}/dashboard/employees`, section: "employees" as const },
  ];
    ...
    >
      <RoleBasedSidebar
        items={menuItems.map((i: any) => ({
          ...
          href: i.link,
          onClick: i.section ? () => {
            setActiveSection(i.section);
            if (i.section === "callers" && callerSubView === "control") setCallerSubView("table");
          } : undefined
        }))}
      ...
      {/* ── MAIN ── */}
      <div className={`flex-1 flex flex-col h-screen overflow-hidden transition-colors duration-300 ${t.mainBg}`}>
        ...
        <header className={`h-16 border-b flex items-center justify-between px-8 z-30 flex-shrink-0 transition-colors duration-300 ${t.header}`}>
          ...
          <div className="flex items-center gap-6">
            ...
            {/* System Settings Icon — visible to org admins only */}
            {isOrgAdmin(user?.role) && (
              <button onClick={() => router.push(`/org/${orgSlug}/dashboard/settings`)} aria-label="Settings"
                className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center cursor-pointer justify-center shadow-sm ${t.toggleBtn}`}>
                ...
                <form onSubmit={handleAddEmployee} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-start">
                  ...
                  <div>
                    ...
                    <select value={role} onChange={e => setRole(e.target.value)} required className={t.sel}>
                      <option value="" disabled className="text-gray-500 bg-white dark:bg-[#1a1a1a]">-- Choose Role --</option>
                      {Array.from(new Set(["Site Head", "Sales Manager", "Admin", "Receptionist", ...dbRoles.map(r => r.name)])).map(roleName => (
                  ...
                  <div className={`flex items-center gap-4 mb-6 border rounded-2xl p-5 ${t.detailCard}`}>
                    ...
                    <div className="flex-1">
                      <h1 className={`text-xl font-bold flex items-center gap-3 ${t.text}`}>
                        <span className={t.accentText}>#{selectedLead.lead_number ?? selectedLead.id}</span>{selectedLead.name}
                      </h1>
                                ...
                                : callerSavedFormLeads.length === 0
                                  ...
                                  : callerSavedFormLeads.map((lead: any) => (
                                    <tr key={lead.id} className={`transition-colors group align-middle ${t.tableRow}`}>
                                      <td className={`px-4 py-3 font-mono font-bold ${t.accentText}`}>#{lead.lead_number ?? lead.id}</td>
                        ...
                        : filtered.slice(0, visibleCount).map((lead: any) => {
                          ...
                          return (
                            <tr key={lead.id} onClick={() => setTicketLead(lead)} className="hover:bg-[#1a1a1a] transition-colors cursor-pointer group align-middle">
                              <td className="px-4 py-3 font-mono text-orange-400 font-bold">#{lead.lead_number ?? lead.id}</td>
                    ...
                    {formLeads.slice(0, visibleCount).map((lead: any) => (
                      <div key={lead.id} onClick={() => setDetailLead(lead)}
                        ...
                        <div className="flex justify-between items-start mb-4 pb-3 border-b border-[#2a2a2a]">
                          <div>
                            <p className="text-[10px] text-orange-400 font-bold mb-1">#{lead.lead_number ?? lead.id}</p>
                      ...
                      <tbody className="divide-y divide-[#1a1a1a]">
                        {interestedLeads.slice(0, visibleCount).map((lead: any) => (
                          <tr key={lead.id} className="hover:bg-[#1a1a1a] transition-colors group align-middle">
                            <td className="px-4 py-3 font-mono text-green-400 font-bold">#{lead.lead_number ?? lead.id}</td>
                      ...
                      <tbody className="divide-y divide-[#1a1a1a]">
                        {notIntLeads.slice(0, visibleCount).map((lead: any) => (
                          <tr key={lead.id} className="hover:bg-[#1a1a1a] transition-colors group">
                            <td className="px-5 py-3 font-mono text-red-400 font-bold">#{lead.lead_number ?? lead.id}</td>
